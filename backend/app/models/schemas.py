...
class ProcessingJob(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    document_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("documents.id"), nullable=False)
    status: Mapped[JobStatus] = mapped_column(Enum(JobStatus, name="job_status", create_type=False, values_callable=lambda objs: [e.value for e in objs]), default=JobStatus.PENDING)
...
class GenerationResult(Base):
    ...
    model_provider: Mapped[ModelProvider] = mapped_column(Enum(ModelProvider, name="model_provider", create_type=False, values_callable=lambda objs: [e.value for e in objs]), nullable=False)
